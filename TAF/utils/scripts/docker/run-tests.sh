...
# Common Variables
USE_DB=-redis
USE_RELEASE=nightly-build
USE_SHA1=master  # developer-script branch or SHA1
...
# Get compose file from developer-script
...
case ${TEST_STRATEGY} in
  1)
    ...
    docker run --rm --network host -v ${WORK_DIR}:${WORK_DIR}:z -w ${WORK_DIR} \
            ...
            -v /var/run/docker.sock:/var/run/docker.sock ${TAF_COMMON_IMAGE} \
            --exclude Skipped --include v2-api -u functionalTest/V2-API -p device-virtual
